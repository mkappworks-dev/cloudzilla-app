...
| File | Action | Purpose |
|---|---|---|
| `migrations/045_create_discussions.sql` | Create | Three tables + indexes + deferred FK |
...
| `internal/router/router.go` | Edit | Register routes; add `"discussions"`, `"discussion_detail"` to `pageNames` |
...
- [ ] Step 1: Create `migrations/045_create_discussions.sql`:
...
- [ ] Step 3: In `internal/router/router.go`, add `"discussions"` and `"discussion_detail"` to `pageNames`:
...
Register the page routes (after the milestones routes):
```go
