...
| File | Action | Purpose |
|------|--------|---------|
| `migrations/036_create_sso_config.sql` | Create | `sso_configs` table + `sso_provider`/`sso_id` columns on `users` |
...
| `internal/handler/viewmodels.go` | Edit | Add `SSOSettingsData` |
| `internal/router/router.go` | Edit | Register SSO routes + add `"sso_settings"` to page names |
| `cmd/server/frontend/templates/pages/sso_settings.html` | Create | Superadmin SSO configuration page |
| `cmd/server/frontend/templates/pages/login.html` | Edit | Add LDAP login form + SAML button when providers are enabled |

---

### Task 1: Database Migration

- [ ] Create `migrations/036_create_sso_config.sql`:
...
- [ ] Edit the existing `LoginData` struct in `viewmodels.go` to add the two SSO fields:
...
func (h *Handler) PageSSOSettings(w http.ResponseWriter, r *http.Request) {
	...
	h.render(w, "sso_settings", SSOSettingsData{
...
- [ ] Create `cmd/server/frontend/templates/pages/sso_settings.html`:

```html
{{define "title"}}SSO Settings{{end}}
...
<div class="space-y-8 max-w-3xl">
  ...
  {{if .Success}}
  <div class="p-4 bg-green-50 border border-green-200 rounded-lg text-green-800 text-sm">{{.Success}}</div>
  {{end}}
  {{if .Error}}
  ...
  <div class="bg-white border border-gray-200 rounded-lg p-6">
    <div class="flex items-center justify-between mb-6">
      ...
      {{if and .LDAPConfig .LDAPConfig.Enabled}}
    ...
    <form method="POST" action="/admin/sso" class="space-y-4">
      ...
      <div class="grid grid-cols-2 gap-4">
        <div>
          ...
          <input type="text" name="ldap_host" placeholder="ldap.example.com"
                 value="{{if .LDAPConfig}}{{index .LDAPConfig.Config "host"}}{{end}}"
                 class="w-full px-3 py-2 border border-gray-300 rounded-md text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
        </div>
        <div>
          <label class="block text-sm font-medium text-gray-700 mb-1">Port</label>
          <input type="text" name="ldap_port" placeholder="389"
                 value="{{if .LDAPConfig}}{{index .LDAPConfig.Config "port"}}{{end}}"
                 class="w-full px-3 py-2 border border-gray-300 rounded-md text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
        </div>
      </div>
      <div>
        <label class="block text-sm font-medium text-gray-700 mb-1">Base DN</label>
        <input type="text" name="ldap_base_dn" placeholder="dc=example,dc=com"
               value="{{if .LDAPConfig}}{{index .LDAPConfig.Config "base_dn"}}{{end}}"
      ...
      <div>
        <label class="block text-sm font-medium text-gray-700 mb-1">Bind DN Template</label>
        <input type="text" name="ldap_bind_dn_tmpl" placeholder="uid=%s,ou=people,dc=example,dc=com"
               value="{{if .LDAPConfig}}{{index .LDAPConfig.Config "bind_dn_tmpl"}}{{end}}"
               class="w-full px-3 py-2 border border-gray-300 rounded-md text-sm font-mono focus:outline-none focus:ring-2 focus:ring-blue-500">
        <p class="text-xs text-gray-500 mt-1">Use <code class="bg-gray-100 px-1 rounded">%s</code> as a placeholder for the username.</p>
      </div>
      <div class="flex items-center gap-3">
        <input type="checkbox" id="ldap-tls" name="ldap_use_tls" value="true"
               {{if and .LDAPConfig (eq (index .LDAPConfig.Config "use_tls") "true")}}checked{{end}}
               class="rounded border-gray-300">
        <label for="ldap-tls" class="text-sm font-medium text-gray-700">Use TLS (LDAPS)</label>
      </div>
      <div class="flex items-center gap-3">
        <label class="block text-sm font-medium text-gray-700">Enable LDAP SSO</label>
        <select name="enabled" class="px-3 py-2 border border-gray-300 rounded-md text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
          <option value="false" {{if or (not .LDAPConfig) (not .LDAPConfig.Enabled)}}selected{{end}}>Disabled</option>
          <option value="true" {{if and .LDAPConfig .LDAPConfig.Enabled}}selected{{end}}>Enabled</option>
        </select>
      </div>
  ...
  <div class="bg-white border border-gray-200 rounded-lg p-6">
    <div class="flex items-center justify-between mb-6">
      ...
      {{if and .SAMLConfig .SAMLConfig.Enabled}}
      <span class="px-3 py-1 bg-green-100 text-green-800 text-sm font-medium rounded-full">Enabled</span>
      {{else}}
      <span class="px-3 py-1 bg-gray-100 text-gray-600 text-sm font-medium rounded-full">Disabled</span>
      {{end}}
    ...
    <form method="POST" action="/admin/sso" class="space-y-4">
      ...
      <div>
        <label class="block text-sm font-medium text-gray-700 mb-1">Entity ID (SP)</label>
        <input type="text" name="saml_entity_id" placeholder="https://cloudzilla.example.com"
               value="{{if .SAMLConfig}}{{index .SAMLConfig.Config "entity_id"}}{{end}}"
               class="w-full px-3 py-2 border border-gray-300 rounded-md text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
      </div>
      <div>
        <label class="block text-sm font-medium text-gray-700 mb-1">IdP SSO URL / Metadata URL</label>
        <input type="url" name="saml_metadata_url" placeholder="https://idp.example.com/sso/saml"
               value="{{if .SAMLConfig}}{{index .SAMLConfig.Config "metadata_url"}}{{end}}"
               class="w-full px-3 py-2 border border-gray-300 rounded-md text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
      </div>
      <div>
        <label class="block text-sm font-medium text-gray-700 mb-1">Assertion Consumer Service URL</label>
        <input type="url" name="saml_acs_url" placeholder="https://cloudzilla.example.com/auth/saml/callback"
               value="{{if .SAMLConfig}}{{index .SAMLConfig.Config "acs_url"}}{{end}}"
               class="w-full px-3 py-2 border border-gray-300 rounded-md text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
      </div>
      <div>
        <label class="block text-sm font-medium text-gray-700 mb-1">IdP Signing Certificate (PEM, no headers)</label>
        <textarea name="saml_idp_cert" rows="4"
                  class="w-full px-3 py-2 border border-gray-300 rounded-md text-sm font-mono focus:outline-none focus:ring-2 focus:ring-blue-500"
                  placeholder="MIIC...base64...">{{if .SAMLConfig}}{{index .SAMLConfig.Config "idp_cert"}}{{end}}</textarea>
      </div>
      <div class="flex items-center gap-3">
        <label class="block text-sm font-medium text-gray-700">Enable SAML SSO</label>
        <select name="enabled" class="px-3 py-2 border border-gray-300 rounded-md text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
          <option value="false" {{if or (not .SAMLConfig) (not .SAMLConfig.Enabled)}}selected{{end}}>Disabled</option>
          <option value="true" {{if and .SAMLConfig .SAMLConfig.Enabled}}selected{{end}}>Enabled</option>
        </select>
      </div>
      <div class="flex gap-3">
        ...
        {{if and .SAMLConfig .SAMLConfig.Enabled}}
        <a href="/auth/saml/metadata" target="_blank"
...
- [ ] Edit `cmd/server/frontend/templates/pages/login.html`. Add SSO buttons below the Google OAuth section:

Replace the closing `</div>` of the card (before the last `{{end}}`) with:

```html
    {{if or .LDAPEnabled .SAMLEnabled}}
    <div class="mt-6">
      <div class="relative">
      ...
      <div class="mt-4 space-y-3">
        {{if .SAMLEnabled}}
        <a href="/auth/saml"
           class="flex w-full items-center justify-center gap-3 rounded-md border border-gray-300 bg-white px-4 py-2 text-sm font-medium text-gray-700 shadow-sm hover:bg-gray-50 transition-colors">
          ...
          <div class="p-4 border-t border-gray-100">
            <form method="POST" action="/auth/ldap" class="space-y-3">
              <div>
                ...
                <input type="text" id="ldap-username" name="username" required
                       class="w-full px-3 py-2 border border-gray-300 rounded-md text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
              </div>
              <div>
                <label for="ldap-password" class="block text-xs font-medium text-gray-700 mb-1">Password</label>
                <input type="password" id="ldap-password" name="password" required
                       class="w-full px-3 py-2 border border-gray-300 rounded-md text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
              </div>
              <button type="submit"
                      class="w-full px-4 py-2 bg-gray-700 text-white rounded-md text-sm font-medium hover:bg-gray-800 transition-colors">
                Sign in with LDAP
              </button>
            </form>
          </div>
...
- [ ] Stage all new and modified files:
  - `migrations/036_create_sso_config.sql`
  ...
  - `cmd/server/frontend/templates/pages/sso_settings.html`
  - `cmd/server/frontend/templates/pages/login.html`
...
- [ ] `sso_settings` is in the `pageNames` slice so the template is compiled at startup.
