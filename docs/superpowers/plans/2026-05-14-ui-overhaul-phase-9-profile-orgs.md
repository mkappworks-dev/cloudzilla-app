...
- **Account settings consolidated into one page at `/settings`**, pulled forward from Phase 10. It is a single scrolling page of sections with a scroll-spy sidebar of `#section` anchors, not tabs. Saved replies and OAuth apps are sections; data export, sessions and emails are marked "Coming soon". `/settings/{security,notifications,tokens,replies,oauth-apps}` 301 to their `#section`. Why: the new profile fields and notification prefs needed an edit surface, and Phase 10 already planned the single page.
...
- **Pins are written by `UserStore.AddPinnedRepo` (row lock) and `RemovePinnedRepo` (one `array_remove`), not `SetPinnedRepoIDs`.** Why: the pin modal sends a request per checkbox without waiting, and a read-modify-write of the whole array lost pins while every request returned 200.
