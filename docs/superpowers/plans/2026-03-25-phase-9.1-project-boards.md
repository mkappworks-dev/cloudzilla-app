...
| File | Action | Purpose |
|---|---|---|
| `internal/db/migrations/037_create_projects.sql` | Create | DB schema for projects, columns, cards |
...
**Files:**
- [ ] Step 1: Create `internal/db/migrations/037_create_projects.sql` with the following content:
...
**Files:**
- [ ] Step 1: In `internal/router/router.go`, add the page routes inside the existing page routes block (after the `/{owner}/{repo}/commit/{sha}` line):
...
- [ ] Step 2: In `internal/router/router.go`, add the API routes inside the `/api/repos` route group (e.g. after the branch protections block):
...
### Task 10: Security review + Pull Request
...
- [ ] Create a pull request: `gh pr create --base main --head feat/phase-9.1-project-boards --title "feat(phase-9.1): add Kanban project boards" --body "Adds per-repository Kanban project boards with columns and cards, supporting issue and PR cards with drag-order persistence."`
...
- [ ] Routes for projects page and project detail page are registered in `router.go`.
