...
func New(services *service.Services, cfg *config.Config, frontend fs.FS) http.Handler {
	...
	r.With(authMW).Get("/settings/organizations", h.PageOrganizations)
	...
	r.With(authMW).Get("/{owner}/{repo}/wiki/new", h.PageWikiNew)
	...
	r.Route("/api/repos", func(r chi.Router) {
		...
		r.Route("/{owner}/{repo}/issues", func(r chi.Router) {
			...
			r.With(authMW).Post("/{number}/priority", h.SetIssuePriority)
			r.Get("/{number}/title", h.IssueTitleSection)
			r.With(authMW).Patch("/{number}/title", h.EditIssueTitle)
			r.Get("/{number}/body", h.IssueBodySection)
			r.With(authMW).Patch("/{number}/body", h.EditIssueBody)
			r.With(authMW).Post("/{number}/linked-pulls/{pullNumber}", h.LinkIssuePull)
			r.With(authMW).Delete("/{number}/linked-pulls/{pullNumber}", h.UnlinkIssuePull)
		...
		// Wiki
		r.With(authMW).Post("/{owner}/{repo}/wiki/order", h.WikiSetPageOrder)
