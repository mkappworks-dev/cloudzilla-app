...
func renderToggle(t *testing.T, attrs templ.Attributes) string {
	t.Helper()
	var buf bytes.Buffer
	if err := Toggle(ToggleDefault, ButtonSizeSM, attrs).Render(context.Background(), &buf); err != nil {
		t.Fatalf("render: %v", err)
	}
	return buf.String()
}

func TestToggle_TypeOverride(t *testing.T) {
	out := renderToggle(t, templ.Attributes{"type": "submit"})
...
func TestToggle_DefaultTypeButton(t *testing.T) {
	out := renderToggle(t, nil)
	if n := strings.Count(out, "type="); n != 1 {
		t.Fatalf("want exactly one type attribute, got %d in %s", n, out)
	}
...
func TestToggle_TypeAndClassOverride(t *testing.T) {
	out := renderToggle(t, templ.Attributes{"type": "submit", "class": "text-destructive"})
	if n := strings.Count(out, "type="); n != 1 {
		t.Fatalf("want exactly one type attribute, got %d in %s", n, out)
	}
	if !strings.Contains(out, `type="submit"`) {
